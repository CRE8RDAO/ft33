...
import { SBRICK_ERC20_CONTRACT_BLOCK, ERC20FRAX_CONTRACT, STAKING_CONTRACT_V1_BLOCK, UNI_BRICKFRAX_PAIR_BLOCK, BRICK_ERC20_CONTRACT, TREASURY_ADDRESS_BLOCK, SBRICK_ERC20_CONTRACT, TREASURY_ADDRESS, UNI_BRICKFRAX_PAIR, WETH_ERC20_CONTRACT, STAKING_CONTRACT_V1 } from './Constants';
import { dayFromTimestamp } from './Dates';
import { toDecimal } from './Decimals';
import { getBRICKUSDRate, getDiscountedPairUSD, getPairUSD, getETHUSDRate } from './Price';
...
export function loadOrCreateProtocolMetric(timestamp: BigInt): ProtocolMetric{
    ...
    if (protocolMetric == null) {
        ...
        protocolMetric.treasuryBrickDaiPOL = BigDecimal.fromString("0")
        protocolMetric.treasuryBrickFraxPOL = BigDecimal.fromString("0")
        protocolMetric.treasuryBrickLusdPOL = BigDecimal.fromString("0")
...
function getTokenStaked(address: string): BigDecimal{
    let brick_contract = OlympusERC20.bind(Address.fromString(BRICK_ERC20_CONTRACT))
    let total_staked = toDecimal(brick_contract.balanceOf(Address.fromString(address)), 9)
...
function getSbrickSupply(transaction: Transaction): BigDecimal{
    ...
    let sbrick_contract_v1 = sOlympusERC20.bind(Address.fromString(SBRICK_ERC20_CONTRACT))
    ...
    if(transaction.blockNumber.gt(BigInt.fromString(SBRICK_ERC20_CONTRACT_BLOCK))){
        ...
        sbrick_supply = sbrick_supply.plus(toDecimal(sbrick_contract_v2.circulatingSupply(), 9))
    }
...
function getMV_RFV(transaction: Transaction): BigDecimal[]{
    ...
    let brickfraxPair = UniswapV2Pair.bind(Address.fromString(UNI_BRICKFRAX_PAIR))
    ...
    if(transaction.blockNumber.gt(BigInt.fromString(UNI_BRICKFRAX_PAIR_BLOCK))){
        brickfraxBalance = brickfraxPair.balanceOf(Address.fromString(treasury_address))
        ...
        if (brickfraxTotalLP.gt(BigDecimal.fromString("0")) &&  brickfraxBalance.gt(BigInt.fromI32(0))){
            brickfraxPOL = toDecimal(brickfraxBalance, 18).div(brickfraxTotalLP).times(BigDecimal.fromString("100"))
        }
    }
...
function getNextBRICKRebase(transaction: Transaction): BigDecimal{
    ...
    if(transaction.blockNumber.gt(BigInt.fromString(STAKING_CONTRACT_V1_BLOCK))){
        let staking_contract_v2 = OlympusStakingV2.bind(Address.fromString(STAKING_CONTRACT_V1))
...
function getAPY_Rebase(sBRICK: BigDecimal, distributedBRICK: BigDecimal): BigDecimal[]{
    let nextEpochRebase = distributedBRICK.div(sBRICK).times(BigDecimal.fromString("100"));
...
function getRunway(sBRICK: BigDecimal, rfv: BigDecimal, rebase: BigDecimal): BigDecimal[]{
    ...
    if(sBRICK.gt(BigDecimal.fromString("0")) && rfv.gt(BigDecimal.fromString("0")) &&  rebase.gt(BigDecimal.fromString("0"))){
...
export function updateProtocolMetrics(transaction: Transaction): void{
    ...
    let apy_rebase = getAPY_Rebase(pm.sBrickCirculatingSupply, pm.nextDistributedBrick)
    pm.currentAPY = apy_rebase[0]
    pm.nextEpochRebase = apy_rebase[1]

    //Runway
    let runways = getRunway(pm.sBrickCirculatingSupply, pm.treasuryRiskFreeValue, pm.nextEpochRebase)
